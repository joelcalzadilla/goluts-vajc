    ...
    private function search_articulos()
    {
        ...
        if ($this->query) {
            ...
            if (is_numeric($query)) {
                ...
                $sql .= "(articulos.referencia = " . $this->empresa->var2str($query)
                    . " OR articulos.referencia LIKE '%" . $query . "%'"
                    . " OR articulos.codbarras LIKE '%" . $query . "%'"
                    //. " OR articulo_codbarras.codbarras LIKE '%" . $query . "%'"
                    . " OR articulos.partnumber LIKE '%" . $query . "%'"
                    . " OR articulos.equivalencia LIKE '%" . $query . "%'"
                    . " OR articulos.descripcion LIKE '%" . $query . "%'"
                    . " OR articulos.codbarras = " . $this->empresa->var2str($query) . ")";
            } else {
                /// ¿La búsqueda son varias palabras?
                $palabras = explode(' ', $query);
                if (count($palabras) > 1) {
                    $sql .= "(lower(articulos.referencia) = " . $this->empresa->var2str($query)
                        . " OR lower(articulos.codbarras) LIKE '%" . $query . "%'"
                        //. " OR lower(articulo_codbarras.codbarras) LIKE '%" . $query . "%'"
                        . " OR lower(articulos.referencia) LIKE '%" . $query . "%'"
                        . " OR lower(articulos.partnumber) LIKE '%" . $query . "%'"
                        . " OR lower(articulos.equivalencia) LIKE '%" . $query . "%'"
                        . " OR (";

                    foreach ($palabras as $i => $pal) {
                        if ($i == 0) {
                            $sql .= "lower(articulos.descripcion) LIKE '%" . $pal . "%'";
                        } else {
                            $sql .= " AND lower(articulos.descripcion) LIKE '%" . $pal . "%'";
                        }
                    }
                    ...
                    $sql .= "(lower(articulos.referencia) = " . $this->empresa->var2str($query)
                        ...
                        . " OR lower(articulos.partnumber) LIKE '%" . $query . "%'"
                        . " OR lower(articulos.equivalencia) LIKE '%" . $query . "%'"
                        . " OR lower(articulos.codbarras) = " . $this->empresa->var2str($query)
                        . " OR lower(articulos.descripcion) LIKE '%" . $query . "%')";
                }
            }
            $where = ' AND ';
        }
        ...
        if ($this->b_codfamilia) {
            if ($this->b_subfamilias) {
                $sql .= $where . "articulos.codfamilia IN (";
                $coma = '';
                ...
                $sql .= $where . "articulos.codfamilia = " . $this->empresa->var2str($this->b_codfamilia);
            }
            $where = ' AND ';
        }

        if ($this->b_codfabricante) {
            $sql .= $where . "articulos.codfabricante = " . $this->empresa->var2str($this->b_codfabricante);
            $where = ' AND ';
        }

        if ($this->b_constock) {
            if ($this->b_codalmacen == '') {
                $sql .= $where . "articulos.stockfis > 0";
            } else {
                $sql .= $where . "articulos.referencia IN (SELECT referencia FROM stocks WHERE cantidad > 0"
        ...
        if ($this->b_publicos) {
            $sql .= $where . "articulos.publico = TRUE";
            $where = ' AND ';
        }

        if ($this->b_bloqueados) {
            $sql .= $where . "articulos.bloqueado = TRUE";
            $where = ' AND ';
        } else {
            $sql .= $where . "articulos.bloqueado = FALSE";
            $where = ' AND ';
        }

        $order = 'articulos.referencia DESC';
        switch ($this->b_orden) {
            case 'stockmin':
                $order = 'articulos.stockfis ASC';
                break;

            case 'stockmax':
                $order = 'articulos.stockfis DESC';
                break;
            ...
            case 'refmax':
                if (strtolower(FS_DB_TYPE) == 'postgresql') {
                    ...
                    $order = 'lower(articulos.referencia) DESC';
                }
                break;

            case 'descmin':
                $order = 'articulos.descripcion ASC';
                break;

            case 'descmax':
                $order = 'articulos.descripcion DESC';
                break;
            ...
            case 'preciomax':
                $order = 'articulos.pvp DESC';
                break;

            default:
            case 'refmin':
                if (strtolower(FS_DB_TYPE) == 'postgresql') {
                    $order = 'articulos.referencia ASC';
                } else {
                    $order = 'lower(articulos.referencia) ASC';
                }
                break;
        }

        $data = $this->db->select("SELECT COUNT(articulos.referencia) as total" . $sql);
        if ($data) {
