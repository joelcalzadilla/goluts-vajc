...
class terminal_caja extends FacturaScripts\model\terminal_caja
{
    ...
    public function imprimir_ticket_tactil(&$factura, &$empresa, $efectivo = NULL, $tarjeta = NULL, $cambio = NULL, $texto_fin = '')
    {
        ...
        $this->add_linea("Comercio: " . $this->sanitize($empresa->nombrecorto) . "\n");
        ...
        $this->add_linea('NIF: ' . $empresa->cifnif . "\n");
        ...
        if ($factura->nombrecliente != "00 Contado") {
        ...
        foreach ($factura->get_lineas() as $col) {
            ...
            $this->add_linea(
                sprintf("%3s", $col->cantidad) . " " . substr($this->sanitize($col->descripcion), 0, $this->anchopapel - 4) . "\n"
            );
            if ($col->dtopor > 0) {
                $this->add_linea(
                    '    ' . sprintf("%-" . $width . "s", 'DESCUENTO: ' . $this->show_numero($col->dtopor) . "%") . "\n"
                );
            }
            $this->add_linea(
    ...
    public function imprimir_extracto($familias, $total, $empresa, $agente, $dia, $hora) {
        ...
        $this->tickets .= $linea_iguales . "\n";
        $this->tickets .= "\n";

        $this->tickets .= $this->sanitize($empresa->nombrecorto) . "\n";
        $this->tickets .=
            $this->sanitize($empresa->direccion) . "\nCP: " . $empresa->codpostal
            . ' - ' . $this->sanitize($empresa->ciudad) . "\n";

        if ($empresa->telefono) {
            $this->tickets .= 'Tlf: ' . $empresa->telefono . "\n\n";
        }

        $this->tickets .=$this->sanitize($empresa->nombre) . '  ';
        $this->tickets .= FS_CIFNIF . ': ' . $empresa->cifnif . "\n";
//        $this->tickets .="Cajero/a: " . $agente . "\n";

        $this->tickets .= "\n=============== EXTRACTOS ==============\n\n";

        $this->tickets .=
            $this->center_text($dia . ' ' . $hora) . "\n" .
            $this->center_text($this->nombre) . "\n";

        $this->tickets .="\n=============== FAMILIAS ===============\n\n";
        $this->tickets .= 'NOMBRE' .
            sprintf('%' . ($this->anchopapel - 6) . 's', 'PRECIO') . "\n";
        $this->tickets .= $linea_iguales . "\n";
        foreach ($familias as $key => $fam) {
            if($fam["ventas"]){
                $this->tickets .=$fam['descripcion'] . sprintf('%' . ($this->anchopapel - strlen($fam['descripcion']) - 4) . 's', $this->show_numero($fam['vendido'])) . " EUR\n";
            }
        }
        $this->tickets .= $linea_iguales . "\n";

        $this->tickets .="\nTOTAL" .
            sprintf('%' . ($this->anchopapel - 9) . 's', $this->show_numero($total)) . " EUR\n\n";
        $this->tickets .= $linea_iguales . "\n";
     ...
     public function imprimir_ventas_familia($familias, $total, $empresa, $agente,$desde,$hasta, $dia, $hora) {
...
//        $this->tickets .="Cajero/a: " . $agente . "\n";
        ...
        $this->tickets .=
              /*imprime el rango de fechas desde hasta*/
            $this->center_text($desde . ' ' . $hasta) . "\n" .
            $this->center_text($dia . ' ' . $hora) . "\n" .
            $this->center_text($this->nombre) . "\n";
