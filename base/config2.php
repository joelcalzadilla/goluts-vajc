...
if (file_exists(FS_FOLDER . '/tmp/' . FS_TMP_NAME . 'enabled_plugins.list')) {
    ...
    $list = explode(',', 'distribucion,nomina,asientos_asistente,tcpdf,presupuestos_y_pedidos,megafacturador,megacopiador,servicios,tpv_tactil,gestion_documental,adminlte,beneficios,facturacion_base');
}
if (!empty($list)) {
    foreach ($list as $f) {
